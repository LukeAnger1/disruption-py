...
class D3DPhysicsMethods:
    ...
    def get_power_parameters(params: PhysicsMethodParams):
        ...
        try:
            ...
            p_nbi = np.zeros(len(params.times))
            params.logger.warning("Failed to open NBI node")
            params.logger.opt(exception=True).debug(e)
    ...
    def get_density_parameters(params: PhysicsMethodParams):
        ...
        try:
            ...
            params.logger.warning("Failed to compute Greenwald fraction.")
            params.logger.opt(exception=True).debug(e)
    ...
    def get_rt_density_parameters(params: PhysicsMethodParams):
        ...
        try:
            ...
            params.logger.warning("Failed to compute Greenwald fraction rt.")
            params.logger.opt(exception=True).debug(e)
    ...
    def get_z_parameters(params: PhysicsMethodParams):
        ...
        try:
            ...
            params.logger.warning("Failed to get efit parameters")
            params.logger.opt(exception=True).debug(e)
    ...
    def get_peaking_factors(params: PhysicsMethodParams):
        ...
        smoothing_window = 40
        p_rad_core_def = (
            0.06  # percentage of DIII-D vertical extent defining the core margin
        )
        ...
        try:
            ...
            params.logger.warning(
                "Failed to get CVA and XDIV from MDSPlus."
        ...
        if calculate_prad_pf:
            try:
                ...
                params.logger.warning("Failed to get bolometer data")
                params.logger.opt(exception=True).debug(e)
    ...
    def _efit_rz_interp(ts, efit_dict):
        ...
        rho_vn_diag = np.full(psin.shape[:2], np.nan)
    ...
    def _get_efit_dict(params: PhysicsMethodParams):
        ...
        for node in nodes:
            try:
                ...
                params.logger.warning(
                    "Failed to get {node} from efit, Setting to all NaNs.",
                    node=node,
                )
                params.logger.opt(exception=True).debug(e)
